...
resource "aws_security_group" "service_access" {
  ...
  revoke_rules_on_delete = true
  vpc_id                 = var.vpc_id
  name                   = module.label_service_access.id
  description            = "Allow inbound traffic from Security Groups and CIDRs for service access in private subnets. Allow all outbound traffic"
  tags                   = module.label_service_access.tags
...
resource "aws_security_group_rule" "service_access_ingress_security_groups" {
  count                    = var.enabled ? length(var.service_access_allowed_security_groups) : 0
  description              = "Allow inbound traffic from Security Groups"
  type                     = "ingress"
  from_port                = 0
  to_port                  = 65535
  protocol                 = "tcp"
  source_security_group_id = var.service_access_allowed_security_groups[count.index]
  security_group_id        = join("", aws_security_group.service_access.*.id)
}

resource "aws_security_group_rule" "service_access_ingress_cidr_blocks" {
  count             = var.enabled && length(var.service_access_allowed_cidr_blocks) > 0 ? 1 : 0
  description       = "Allow inbound traffic from CIDR blocks"
  type              = "ingress"
  from_port         = 0
  to_port           = 65535
  protocol          = "tcp"
  cidr_blocks       = var.service_access_allowed_cidr_blocks
  security_group_id = join("", aws_security_group.service_access.*.id)
}
...
resource "aws_emr_cluster" "default" {
  ...
  ec2_attributes {
    ...
    emr_managed_master_security_group = join("", aws_security_group.master.*.id)
    emr_managed_slave_security_group  = join("", aws_security_group.slave.*.id)
    service_access_security_group     = join("", aws_security_group.service_access.*.id)
