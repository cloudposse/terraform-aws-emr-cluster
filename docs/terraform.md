...
| kerberos_attributes | Kerberos configuration for the cluster | object | `null` | no |
...
| master_instance_group_autoscaling_policy | String containing the EMR Auto Scaling Policy JSON for the Master instance group | string | `null` | no |
...
| step | List of steps to run when creating the cluster. Defined below. It is highly recommended to utilize the lifecycle configuration block with ignore_changes if other steps are being managed outside of Terraform. This argument is processed in attribute-as-blocks mode | object | `null` | no |
